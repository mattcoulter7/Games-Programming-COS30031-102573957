...
struct PolygonCollider : public Collider2D {
	PolygonCollider() : Collider2D() {};
	PolygonCollider(const PolygonCollider& obj) : Collider2D(obj) {
		localPts = obj.localPts;
		worldPts = obj.worldPts;
		edges = obj.edges;
	};
	PolygonCollider(json obj) : Collider2D(obj) {
        if (!obj["localPts"].is_null()) {
            for (json v : obj["localPts"]) {
                float x = v["x"].is_null() ? 0 : v["x"];
                float y = v["y"].is_null() ? 0 : v["y"];
                localPts.push_back(Vector2{ x,y });
            }
        }
	}
	~PolygonCollider() {}

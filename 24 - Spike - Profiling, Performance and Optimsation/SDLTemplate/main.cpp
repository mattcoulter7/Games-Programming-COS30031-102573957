...
void render_box(SDL_Rect &rect, SDL_Renderer* renderer, SDL_Color& color)
{
...
void render_boxes(SDL_Renderer* renderer)
{
	...
	for (int i = 0; i < BOX_COUNT; i++) {
		if (boxes[i].state == CONTACT_NO) {
			render_box(boxes[i].rect, renderer, white);
		}
		else {
			render_box(boxes[i].rect, renderer, red);
		}
	}
}
...
void update_boxes()
{
	...
	for (int i = 0; i < BOX_COUNT; i++) {
		...
		else if (boxes[i].y >= SCREEN_HEIGHT) boxes[i].y -= SCREEN_HEIGHT;
	...
	for (int i = 0; i < BOX_COUNT; i++) {
		...
		// update the rendering box world position
		boxes[i].rect.x = boxes[i].x;
		boxes[i].rect.y = boxes[i].y;
		boxes[i].rect.w = boxes[i].w;
		boxes[i].rect.h = boxes[i].h;
	}
}
...
int run_test(const char* title, void (*function_ptr)()) {
	...
	while (running) {
		...
		update_boxes();
		// 5. call whatever function has been set to test all i against j boxes
		crash_test_all_ptr();
...
{
	int result;
	/*result = run_test("Test A1", crash_test_all_A1);
	if (result != 0) return result;
	result = run_test("Test A2", crash_test_all_A2);
	if (result != 0) return result;
	result = run_test("Test B", crash_test_all_B);
	if (result != 0) return result;
	result = run_test("Test C", crash_test_all_C);
	if (result != 0) return result;*/
	result = run_test("Test E", crash_test_all_E);
	if (result != 0) return result;
	result = run_test("Test D", crash_test_all_D);
	if (result != 0) return result;
	result = run_test("Test D", crash_test_all_D);
	if (result != 0) return result;
